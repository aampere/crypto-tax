...
class MarketInfo:
    def __init__(self, p_c, a_c): #pass in intiated public and authenticated clients
        ...
        self.productids = []
        self.biproductids = []
        self.alltriangles = [] #all triangles, with no equivalents, but no order guaranteed
        self.usdtriangles = []
        ...
        for p in self.products:
            ...
            if p['post_only']==True or p['limit_only']==True or p['cancel_only']==True or p['trading_disabled']==True or p['status']!='online':
        ...
        while i!=len(self.biproductids):
            ...
            while j!=len(self.biproductids):
                if self.biproductids[i].split('-')[1]==self.biproductids[j].split('-')[0] and (self.biproductids[j].split('-')[1]+'-'+self.biproductids[i].split('-')[0]) in self.biproductids:
                    pa0 = ProductAction(self.biproductids[i],self.productids)
                    pa1 = ProductAction(self.biproductids[j],self.productids)
                    pa2 = ProductAction(self.biproductids[j].split('-')[1]+'-'+self.biproductids[i].split('-')[0],self.productids)
                    tri = Triangle(pa0,pa1,pa2)
                    if (not tri.reorderSyn(1) in self.alltriangles) and (not tri.reorderSyn(2) in self.alltriangles):
    ...
    def productOK(self, pid):
        ...
        ok = p.get('post_only',True)==False and p.get('limit_only',True)==False and p.get('cancel_only',True)==False and p.get('status','')=='online'
        return ok
    def triangleOK(self, triangle):
        return self.productOK(triangle.pa0.trueid) and self.productOK(triangle.pa1.trueid) and self.productOK(triangle.pa2.trueid)
...
class ProductAction:
    ...
    def __init__(self, ids, fulltrueidlist):
        ...
        if self.nameid in self.trueidlist:
            ...
            print('Supplied nameid %s or its reverse could not be found in the product list' % self.nameid)
...
class Triangle:
    ...
    def __eq__(self, other): #This checks for identical order too. equal cycles with different starting currencies are inequal according to this
        if isinstance(other,Triangle):
            return self.tri[0]==other.tri[0] and self.tri[1]==other.tri[1] and self.tri[2]==other.tri[2]
        return False
    def hasCurrency(self,cur):
        for pa in self.tri:
    ...
    def shortIdChain(self):
        return self.tri[0].nameid.split('-')[0] + '-' + self.tri[1].nameid + '-' + self.tri[2].nameid.split('-')[1]
    def printIDs(self):
...
def trianglesEqual(tri1,tri2):
    return (tri1[0]==tri2[0] and tri1[1]==tri2[1] and tri1[2]==tri2[2]) or (tri1[0]==tri2[1] and tri1[1]==tri2[2] and tri1[2]==tri2[0]) or (tri1[0]==tri2[2] and tri1[1]==tri2[0] and tri1[2]==tri2[1])
...
#Misc market info requests/calculations
...
def getBestLimitBuyInfo(mi, pid, funds, level2book=0):
    ...
    if bestaskprice-bestbidprice>5.0*quoteincrement: #If the spread is greater than eg 0.05 USD, bid two cents higher
        limitbidprice = bestaskprice-2.0*quoteincrement
    limitbidprice_str = reducePrecision(str(limitbidprice), mi.productsdict[pid]['quote_increment'])
    limitbidprice = float(limitbidprice_str)
    limitbidvolume = funds/limitbidprice
    limitbidvolume_str = reducePrecision(str(limitbidvolume),mi.productsdict[pid]['base_increment'])
...
def bestLimitBuy(mi, pid, funds, level2book=0, coid=''):
    ...
    return mi.auth_client.buy(product_id=pid, order_type='limit', size=limitbidvolume_str, price=limitbidprice_str, client_oid=coid)
def getBestLimitSellInfo(mi, pid, size, level2book=0):
    ...
    limitaskvolume_str = reducePrecision(str(size), mi.productsdict[pid]['base_increment'])
...
def bestLimitSell(mi, pid, size, level2book=0, coid=''):
    ...
    return mi.auth_client.sell(product_id=pid, order_type='limit', size=limitaskvolume_str, price=limitaskprice_str, client_oid=coid)        
