...
class CryptoTax:
    def __init__(self, coinbase_key, coinbase_b64secret, coinbase_passphrase):
        ...
        self.auth_client = cbpro.AuthenticatedClient(coinbase_key, coinbase_b64secret, coinbase_passphrase)
        ...
        with open(fillspath, newline='') as fillscsv:
            ...
            for row in fillsreader:
                ...
                if quotecurrency=='USD' and side=='BUY':
                    ...
                    self.addToHoldings(basecurrency, float(row['size']), -float(row['total']),row['yyyy'],row['mm'],row['dd']) #Total is how much USD we spent, including fee, to procure size
                    ...
                    holdinglist = self.pullFromHoldings(quotecurrency,-float(row['total'])) #total is how much BTC we spent, including fee, to procure size of other currency
                    
                    basecurrencyprice = self.closestPrice(basecurrency,pricelogs[basecurrency],float(row['timestamp']))
                    ...
                    self.transactions.append({'description':desc,'dateacquired':dateacquired,'datesold':datesold,'proceeds':usdvalueofcrypto,'cost':totalbasisbtc,'gain':usdvalueofcrypto-totalbasisbtc,'row':row,'holdinglist':holdinglist})
                    
                    #Finally, add the new currency to our holdings
                    self.addToHoldings(basecurrency, float(row['size']), usdvalueofcrypto, row['yyyy'],row['mm'],row['dd'])
                    ...
                    holdinglist = self.pullFromHoldings(basecurrency,float(row['size'])) #size is how much crypto we sold.
                    
                    quotecurrencyprice = self.closestPrice(quotecurrency,pricelogs[quotecurrency],float(row['timestamp']))
                    ...
                    self.transactions.append({'description':desc,'dateacquired':dateacquired,'datesold':datesold,'proceeds':usdvalueofquotecurrency,'cost':totalbasiscrypto,'gain':usdvalueofquotecurrency-totalbasiscrypto,'row':row,'holdinglist':holdinglist})
                    
                    #Finally, add the new BTC to our holdings
                    self.addToHoldings(quotecurrency, float(row['total']), usdvalueofquotecurrency, row['yyyy'],row['mm'],row['dd'])
                    ...
                    holdinglist = self.pullFromHoldings(basecurrency,float(row['size'])) #
                    ...
                    datesold = '{0}/{1}/{2}'.format(row['mm'],row['dd'],row['yyyy'])
                    desc = '{0} {1} (virtual currency)'.format(row['size'], basecurrency)
                    self.transactions.append({'description':desc,'dateacquired':dateacquired,'datesold':datesold,'proceeds':totalusd,'cost':totalbasiscrypto,'gain':totalusd-totalbasiscrypto,'row':row,'holdinglist':holdinglist})
    ...
    def addToHoldings(self, curr, size, usdbasis, yyyy, mm, dd):
        ...
        if not curr in self.holdings:
            self.holdings[curr] = [] #if there is no list of holdings yet for this currency, create it.
        self.holdings[curr].append({'size':size, 'usdbasis':usdbasis, 'yyyy':yyyy, 'mm':mm, 'dd':dd})
    ...
    def pullFromHoldings(self, curr, amt):
        ...
        holding = self.holdings[curr] #list of holdings in the currency. earliest acquired is index 0.
    ...
    def getHistoricPrice(self, curr, timestamp):
        ...
        info = self.mi.auth_client.get_product_historic_rates(curr+'-USD',start=datetime.datetime.utcfromtimestamp(timestamp-60).isoformat(),end=datetime.datetime.utcfromtimestamp(timestamp).isoformat(), granularity=60)
        if type(info)==list:
            if len(info)==1:
                candle = info[0]
        ...
        info = self.mi.auth_client.get_product_historic_rates(curr+'-USD',start=datetime.datetime.utcfromtimestamp(timestamp-1200).isoformat(),end=datetime.datetime.utcfromtimestamp(timestamp+1200).isoformat(), granularity=60)
        if type(info)==list:
            if len(info)==0:
                print('the list thats supposed to have historical data in it is empty')
            else:
        ...
        info = self.mi.auth_client.get_product_historic_rates(curr+'-USD',start=datetime.datetime.utcfromtimestamp(timestamp-45000).isoformat(),end=datetime.datetime.utcfromtimestamp(timestamp+45000).isoformat(), granularity=3600)
        if type(info)==list:
            if len(info)==0:
                print('the list thats supposed to have historical data in it is empty')
            else:
